import { Box, Typography, Stack, useMediaQuery, Divider, Button } from '@mui/material';
...
const DetailOrder = ({id}) => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            setCustomizeId(`${date.getFullYear().toString().slice(-2)}.${date.getMonth() < 10 ? `0${date.getMonth()}` : date.getMonth()}.${response.data.id_pesanan}`);
        };
        fetchData();
    }, []);
        ...
        <Box my={3}>
            <ChildCard>
                <Box p={2}>
                    ...
                    <Divider sx={{ my: 2 }} />
                    {data.detail_pesanan && data.detail_pesanan.map((detail) => {
                    ...
                    <Stack direction="row" justifyContent="space-between" mb={3}>
                        ...
                        <Typography variant="h6">
                            {data.ongkir ? new Intl.NumberFormat('id-ID', {
                                style: 'currency',
                                currency: 'IDR',
                            }).format(data.ongkir) : 'Ongkir Belum Dimasukkan'}
                        </Typography>
                    </Stack>
                    <Stack direction="row" justifyContent="space-between" mb={3}>
                        <Typography variant="h6" fontWeight={400}>
                    ...
                    <Stack direction="row" justifyContent="space-between" mb={1}>
                        ...
                        <Typography variant="h5" color="success">
                            {new Intl.NumberFormat('id-ID', {
                                style: 'currency',
                                currency: 'IDR',
                            }).format(data.total_harga + data.ongkir - data.poin_dipakai*100)}
                        </Typography>
                    </Stack>
                </Box>

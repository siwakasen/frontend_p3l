...
export const ProdukDetail = ({ item, kategori, items }) => {
  ...
  function handleBeli(statusPesanan) {
    if (isLogin.data?.role === "User") {
      if (statusPesanan == null) return toastWarning("Pilih jenis pesanan");

      switch (statusPesanan) {
        case "PO":
          if (limit <= 0 && !item.id_hampers && item.id_kategori != 4)
            return toastWarning("Kuota produk tidak mencukupi");
          break;
        case "Ready":
          if (item.stok_produk <= 0)
            return toastWarning("Stok produk tidak mencukupi");
          break;
      }
      ...
      router.push(
        `/checkout?${
          item.id_produk
            ...
            : `hampers=${item.id_hampers}`
        }&date=${value.format("YYYY-MM-DD")}&statusPesanan=${statusPesanan}`
      );
    } else {
  ...
  function ButtonDatePicker(props) {
    ...
    const disablePastDates = (date) => {
      return item.id_kategori == 4 || item.stok_produk > 0
      ...
      {true ? (
        <>
          ...
          <Grid container spacing={1} sx={{ marginBottom: "1rem" }}>
            ...
            {item.nama_produk && item.id_kategori != 4 && (
              <>
                <Grid
                  container
                  item
                ...
                >
                  ...
                  <Grid item xs={9}>
                    <Select
                      ...
                      sx={{ width: "50%" }}
                    >
                      {limit > 0 && <MenuItem value="PO">PO</MenuItem>}
                      {item.stok_produk > 0 && (
          ...
          <Grid container spacing={2} mt={1}>
            <Grid item xs={12} lg={4} md={6}>
              <Button
                ...
                fullWidth
                variant="contained"
                onClick={() => handleBeli(selectValue)}
              >
                Buy Now
              </Button>
            </Grid>
